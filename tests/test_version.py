...
def test_version():
    ...
    setup_version = Query(setup).find_function_call(func_name="setup").extended_node().keyword_args["version"].literal_eval()
    version_version = Query(version).find_assignment("__version__").value.literal_eval_for_node()
